...
//              +《铸造手册》熔炼卷，中频炉实践取中值；无独立 confidence 字段，
//              各默认值=手册典型值（经验参数），视图层逐格可改
// 可疑值说明：FeMo 库内"50~55%"偏低、网络"Cr 烧损39.4%"与惯例矛盾 → 本表不用
// 61.txt（批次2 双AI审计）：字段语义=「铁液总重」指目标出炉铁液量（方案A 口径）——
//   金属炉料 % 与合金/处理剂 kg 均按此量折算成分（calcs 分母=totalWt）；
//   实投总料 = 炉料 kg + 辅料 kg 会略高于出炉量，差额=熔炼损耗，详见视图说明区。
// 备注（61 核查，数值未动）：CE 区间（材料卡）与目标成分存在口径差——HT150 按
//   CE=C+0.33(Si+P) 实算 4.46，超出其声明区间 3.9~4.3；QT 系列区间按"成铁（含处理
//   带入硅）"口径。UI 对不自洽情况显示警示行，牌号数值留给人工裁决。
// ============================================================
...
/** 回炉料吸收率 UI 默认（0.95 > 生铁/废钢 0.90：本厂熟料、少烧损）——视图 loadDefaults 使用 */
export const CHARGE_RET_ABS = 0.95;
