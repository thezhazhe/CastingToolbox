...
export const SIZE_BUCKET = {
  s: '轮廓≤200mm', m: '轮廓200~500mm', l: '轮廓＞500mm',
};
...
/* ---- 起模斜度：JB/T 5105-2022（单位：分 ′，1°=60′）----
   表结构（2022 目次级确认）：表1 黏土砂造型外表面 / 表2 黏土砂造型凹处内表面（独立表，
   值约为表1 的 1.7~2.0 倍）/ 表3 自硬砂造型外表面。选取规则（1991 第 4 章/2022 第 5 章多源
   引述一致）：自硬砂造型时模样【凹处内表面】"允许"按表3 所列数值【再增加 50%】选取——
   系上限授权而非必须 ×1.5（本工具按允许上限取值作推荐，工程取宁大勿小方向）；凹处过深
   时用活块或砂芯形成；黏土砂凹处内表面不走 50% 规则、查独立表2。标准另注：起模困难可
   加大但不超表值一倍；机器造型压实比 >700kPa 时允许再增（旧版上限 ≤50%，2022 原文表述
   未核）。金属/塑料模档止于 1000mm；木模 1991 版另有 >1000~2500 续档（2022 修订未核，
   未建档，>1000mm 给末档参考值）。
   61/62 批次核查：本表数值与公开的 1991 版表值（两个独立转述来源 + 手册）逐档一致；
   2022 官方全文未及——若有修订以标准原文为准。 */
...
// 粘土砂（表1/表2）：金属模与木模分开（导出供 62 核验测试逐档比对）
export const DRAFT_OUTER = { '金属/塑料': [140, 70, 30, 25, 20, 20, 20, 15], 木模: [175, 85, 40, 30, 25, 25, 20, 20] };
export const DRAFT_INNER = { '金属/塑料': [275, 140, 65, 45, 40, 40, 35, 30], 木模: [345, 170, 75, 55, 45, 45, 40, 35] };
// 自硬砂（表3 外表面）：凹处内表面按表3值再增加 50%（上限授权，工具按上限取值）
export const DRAFT_SELF_OUTER = { '金属/塑料': [210, 110, 50, 35, 30, 30, 25, 20], 木模: [240, 125, 55, 40, 35, 35, 30, 25] };
...
export function draftAngles(height, mold, sandType = '潮模砂（湿型）') {
  ...
  return {
    ...
    outOfRange: h > DRAFT_HEIGHTS[DRAFT_HEIGHTS.length - 1],   // 起模面高 >1000mm 超表：末档仅参考
  };
}
